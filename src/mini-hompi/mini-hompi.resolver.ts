...
import {
  ...
  CreateMiniHompiOutput,
} from './dtos/createMiniHompi.dto';
import {
  GetAllMiniHompiInput,
  GetAllMiniHompiOutput,
} from './dtos/getAllMiniHompis.dto';
...
export class MiniHompiResolver {
  constructor(private miniHompiService: MiniHompiService) {}

  @Mutation((returns) => CreateMiniHompiOutput)
  @UseGuards(GqlAuthGuard)
  async createMiniHompi(
    @Args('input') createMiniHompiInput: CreateMiniHompiInput,
    @CurrentUser() owner,
  ): Promise<CreateMiniHompiOutput> {
  ...
  async getMiniHompi(
    @Args('input') getMiniHompiInput: GetMiniHompiInput,
  ): Promise<GetMiniHompiOutput> {
